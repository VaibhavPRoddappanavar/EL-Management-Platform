...
import { CheckCircle, Crown, PlusCircle, UserPlus, Users, XCircle } from 'lucide-react';
...
const TeamManagementView = ({ team }) => {
  ...
  const handleInviteMember = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:5000/${team._id}/invite`, {
      ...
      const response = await axios.get(`http://localhost:5000/${team._id}/invites`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleInviteResponse = async (invite, status) => {
    try {
      ...
      if (status === 'ACCEPTED') {
        ...
        const position = prompt(`Choose a position for ${invite.email}:`, 
          POSITION_OPTIONS.find(pos => 
            !teamData.members.some(member => member.position === pos)
          ) || ''
        );
        ...
        response = await axios.put(`http://localhost:5000/handle/${team._id}/${invite._id}`, {
          status: 'ACCEPTED',
          position
        }, {
          headers: { 'Authorization': `Bearer ${token}` }
  ...
  const handleFinalizeTeam = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`http://localhost:5000/${team._id}/finalize`, {}, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      alert(response.data.message);
    } catch (error) {
  ...
  const handleCancelInvite = async (email) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:5000/${team._id}/cancel-invite`, 
        { email },
        { headers: { 'Authorization': `Bearer ${token}` }}
      );
      const response = await axios.get(`http://localhost:5000/${team._id}/invites`, {
  ...
  const handleDeleteMember = async (memberId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/${team._id}/members/${memberId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      // Refresh members list after deletion
      const membersResponse = await axios.get(`http://localhost:5000/${team._id}/members`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      setTeamData(membersResponse.data);
    } catch (error) {
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-gray-800">{teamLeader.TeamLeaderName}</span>
                </div>
                <span className="text-sm text-gray-500">{teamLeader.TeamleaderEmailID}</span>
                <span className="text-xs text-gray-400">USN: {teamLeader.TeamLeaderUSN}</span>
              </div>
        ...
        {inviteList.map((invite) => (
          <div key={invite._id} 
            ...
            <div className="flex flex-col justify-between items-center">
              <div className="flex flex-col justify-between items-center">
                <span className="font-semibold text-gray-800">{invite.email}</span>
